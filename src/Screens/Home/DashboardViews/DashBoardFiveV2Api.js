...
const DashBoardFiveV2Api = ({
            ...
            <Text
              ...
              {getBundleId() == appIds.quickLube
                ? item?.data?.length > 1
                  ...
                  : strings.BOOK_HERE
                : `${strings.EXPLORE_STORES} ${(appMainData?.is_admin && getBundleId()==appIds.farmersFox)? appData?.profile?.preferences?.super_vendor_nomenclature: appData?.profile?.preferences?.vendors_nomenclature}`}
            </Text>
  ...
  const BannersView = ({
    ...
    let myBanner = item?.banner_image || appMainData?.mobile_banners || appData?.mobile_banners || []
    return (
      !isEmpty(myBanner) ?
