...
import {TaxiHomeDashbord} from '../DashboardViews/Index';
...
export default function TaxiHomeScreen({route, navigation}) {
  ...
  const [showfirstorder, setshowfirstorder] = useState(false);
   const [bannerUrl, setsbannerUrl] = useState(null);
  const [oneTimeShow, setoneTimeShow] = useState(true);
  ...
  const _getAllNearHosptital = async () => {
    const res = await getNearByPlacesMarker(
      locationObj?.latitude,
      locationObj?.longitude,
      '1500',
      ['restaurant'],
      'AIzaSyDHPQM4OBs2I7ngqFOWk0Wk9Ke4AA034VI',
    );
    console.log(res, 'res>>>>>>>>>');
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    } else if (redirectedFrom == 'from_deepLinking_link') {
      // navigation.navigate(navigationStrings.LEADER_BOARD);
      actions
        .testLink(
          `/${referralCode}`,
          {},
        ...
        .then(res => {
          console.log(res, 'erreeseseserrr');
          updateState({referalResponse: res});
  ...
  const homeData =  async() => {
    ...
    actions
      ...
      .then(res => {
        if (!!userData?.auth_token && !showfirstorder && oneTimeShow) {
          setTimeout(() => {
            setshowfirstorder(res?.data?.show_order_popup);
            setsbannerUrl(res?.data?.welcome_banner || null)
    ...
    <View
     ...
     {!!bannerUrl ? <Modal
        ...
        <View
          ...
          <View style={{borderRadius: moderateScale(8), overflow: 'hidden'}}>
            <Image
        ...
        <TouchableOpacity
          onPress={() => setshowfirstorder(false)}
          style={{alignSelf: 'center', marginTop: moderateScaleVertical(10)}}>
          <Image source={imagePath.crossB} style={{tintColor: colors.white}} />
        </TouchableOpacity>
      </Modal> : null}
    </View>
  );
